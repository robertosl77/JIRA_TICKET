create or replace 
FUNCTION       CANTIDAD_LLAMADAS_EFECTIVAS (P_FECHA   IN VARCHAR2,
                                                    P_TIPO    IN VARCHAR2)
   RETURN NUMBER
AS
   v_cant   NUMBER;
...
--             AND TO_CHAR (d.fecha_inicio_doc, 'dd/mm/yyyy') = p_fecha
             AND TO_CHAR (n.fechahora, 'dd/mm/yyyy') = p_fecha
   ...
   ELSE
      SELECT COUNT (*)
       ...
       WHERE     n.EFECTIVO = 1
             AND N.ID_TIPO_NOTA = 4
--             AND d.FECHA_INICIO_DOC BETWEEN TRUNC (SYSDATE - 7) AND TRUNC (SYSDATE + 1)
             AND N.FECHAHORA BETWEEN TRUNC (SYSDATE - 7) AND TRUNC (SYSDATE + 1)
             AND d.TIPO_CORTE = p_tipo;
   END IF;

   RETURN v_cant;
END CANTIDAD_LLAMADAS_EFECTIVAS;