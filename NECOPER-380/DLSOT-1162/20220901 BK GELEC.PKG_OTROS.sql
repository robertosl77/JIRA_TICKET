...
AS
   ...
   BEGIN
      ...
      UPDATE GELEC.ED_CONTACTOS_CLIENTES c
         SET c.CUENTA = P_DESTINO
       WHERE     c.CUENTA = P_ORIGEN
             AND c.LOG_HASTA IS NULL
             AND (   (SELECT COUNT (*)
                        FROM GELEC.ED_NOTAS n
                       WHERE n.IDDESTINO = c.ID_TEL AND n.EFECTIVO = 1) != 0
                  OR (SELECT COUNT (*)
                        FROM GELEC.ED_NOTAS n
                       WHERE n.IDDESTINO = c.ID_TEL AND n.EFECTIVO = 0) < 3);

      -- Migro las notas de hace 6 meses
      UPDATE GELEC.ED_CLIENTE_NOTA c
         SET c.cuenta = P_DESTINO
       WHERE     c.cuenta = P_ORIGEN
             AND (SELECT ADD_MONTHS (l.FECHA, 6)
                    FROM GELEC.ED_LOG l
                   WHERE l.LOG_ID = c.LOG_DESDE) >= SYSDATE;
      ...
      UPDATE GELEC.ED_MARCA_CLIENTE m
         SET m.CUENTA = P_DESTINO
       WHERE     m.CUENTA = P_ORIGEN
             AND (SELECT COUNT (*)
                    FROM GELEC.ED_MARCA_CLIENTE mm
                   WHERE     mm.CUENTA = P_DESTINO
                         AND mm.ID_MARCA = m.ID_MARCA
                         AND mm.LOG_HASTA IS NULL) = 0
             AND m.LOG_HASTA IS NULL;
      ...
      UPDATE GELEC.ED_PACIENTE_CLIENTE p
         SET p.CUENTA = P_DESTINO
       WHERE     p.cuenta = P_ORIGEN
             AND (SELECT COUNT (*)
                    FROM GELEC.ED_PACIENTE_CLIENTE pp
                   WHERE     pp.CUENTA = P_DESTINO
                         AND pp.DNI = p.DNI
                         AND pp.LOG_HASTA IS NULL) = 0
             AND p.LOG_HASTA IS NULL;
