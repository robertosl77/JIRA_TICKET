...
AS
   ...
   BEGIN
      ...
      --verifica si existe relacion cuenta/documento existente
      SELECT COUNT (*)
      ...
      IF V_EXISTE_RELACION = 0 THEN
         ...
         ELSE
            ...
            IF V_EXISTE_CLI_GELEC >0 THEN
                --SI EXISTE CLIENTE ACTUALIZO LOG HASTA CON NULL POR SI ESTABA DADO DE BAJA
                BEGIN
                    UPDATE GELEC.ED_CLIENTES SET LOG_HASTA=NULL WHERE CUENTA=V_NRO_CLIENTE;
                    V_RESULTADO:='OK';
                EXCEPTION
                    WHEN OTHERS THEN
                        V_RESULTADO:='ERROR AL ACTUALIZAR LOG_HASTA';
                END;    
            ELSE
               ...
               IF V_EXISTE_CLI_NEXUS > 0 THEN
                  ...
                  INSERT_CLIENTE_MANUAL (P_NRO_CLIENTE,
                                         ...
                                         V_INSERT_MANUAL);
                                                                  ...
                                                                  DBMS_OUTPUT.PUT_LINE('mi prueba: '||V_INSERT_MANUAL);
                  IF V_INSERT_MANUAL='OK' THEN
                      V_RESULTADO:='OK';
                  ELSE
                      V_RESULTADO:='ERROR AL INSERTAR CLIENTE PROVISORIO: '|| P_NRO_CLIENTE;
                  END IF;
            ...
            IF V_RESULTADO='OK' THEN
                --NECOPER-378 RSLEIVA... exista o no el cliente, se verifica si esta en tabla limite
                GELEC.INSERT_CLIENTE_MANUAL_LIMITE( V_NRO_CLIENTE, 'Batch', V_RESULTADO );
                
                IF V_RESULTADO<>'OK' THEN
                    V_RESULTADO := 'ERROR AL VERIFICAR FECHA LIMITE: ' || V_NRO_CLIENTE;
                ELSE
                    V_RESULTADO := 'OK';
                END IF;
                --NECOPER-378 Fin
                DBMS_OUTPUT.PUT_LINE(V_RESULTADO||' '||v_nro_cliente);             
