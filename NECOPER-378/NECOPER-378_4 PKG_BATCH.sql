...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Package Body PKG_BATCH
--------------------------------------------------------
...
AS
   ...
   IS
       ...
       *	FECHA: 23/12/2022                                            *
       *    USUARIO: RSLEIVA                 	                         *
       *    CAMBIO: Se reemplazo la seccion de baja de clientes EDP por  *
       *    el nuevo proceso GELEC.DELETE_CLIENTE_MANUAL_LIMITE          *
       *    NECOPER: NECOPER-378                                         *
       *                                                                 *
   ...
   BEGIN
      ...
      --NECOPER-378 RSLEIVA 23/12/2022
      --Se reemplaza el proceso anterior de baja de cliente EDP por el nuevo proceso DELETE_CLIENTE_MANUAL_LIMITE
--      UPDATE GELEC.ED_CLIENTES c
...
--and c.log_hasta is null;
      DELETE_CLIENTE_MANUAL_LIMITE( V_LOGID, P_RESULTADO);
      --NECOPER-378 FIN
   ...
   IS
      ...
      -- Variables por fecha de inicio superior a fecha de fin
      V_TMP_FINI NEXUS_GIS.OMS_AFFECT_RESTORE_OPERATION.time%type;
      V_TMP_FFIN NEXUS_GIS.OMS_AFFECT_RESTORE_OPERATION.time%type;
   BEGIN
      v_resultado := 'OK';
      ...
      LOOP
         ...
         DBMS_OUTPUT.PUT_LINE(DOCUMENTO.FECHA_INICIO);
         DBMS_OUTPUT.PUT_LINE(DOCUMENTO.FECHA_RESTAURACION);
